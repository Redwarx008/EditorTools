...
bool PagedTextureProcess(float* data, const ProcessInfo& info);
...
void WriteHeader(FILE* f, int width, int height, int tileSize, int bitDepth, int nChannel, int mipmapCount, int borderWidth)
{
    if (f == nullptr) return;

    uint16_t sizeInfo[] = { (uint16_t)width, (uint16_t)height, (uint16_t)tileSize };
    uint8_t pixelInfo[] = { (uint8_t)bitDepth, (uint8_t)nChannel, (uint8_t)mipmapCount, (uint8_t)borderWidth};
...
extern "C" __declspec(dllexport) bool Create(Config config)
{
    ...
    bool ret = PagedTextureProcess(heights, info);
    if (!ret)
    {
        return false;
    }
...
bool PagedTextureProcess(float* data, const ProcessInfo& info)
{
    ...
    int borderWidth = info.borderWidth;
    ...
    WriteHeader(f, width, height, tileSize, outBitDepth, nChannel, mipLevel, borderWidth);

    float* inData = data;

    PagedTexture* outTiledBitmap = PagedTexture::Create(inData, nChannel, outBitDepth, width, height, tileSize, borderWidth);
    ...
    for (int mip = 1; mip <= mipLevel; ++mip)
    {
        ...
        outTiledBitmap = PagedTexture::Create(outData, nChannel, outBitDepth, w, h, tileSize, borderWidth);
        outTiledBitmap->SaveData(f);
        delete outTiledBitmap;
