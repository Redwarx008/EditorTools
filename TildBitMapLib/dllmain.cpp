...
bool TiledBitmapProcess(float* data, const ProcessInfo& info)
{
    ...
    TiledBitmap* outTiledBitmap = TiledBitmap::Create(inData, nChannel, outBitDepth, width, height, tileSize, borderWidth);
    ...
    for (int mip = 1; mip <= mipLevel; ++mip)
    {
        ...
        outTiledBitmap = TiledBitmap::Create(outData, nChannel, outBitDepth, w, h, tileSize, borderWidth);
        outTiledBitmap->SaveData(f);
        delete outTiledBitmap;
